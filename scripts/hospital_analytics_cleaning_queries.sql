...
SELECT
	...
	deathdate::DATE,
	ssn,
	COALESCE(drivers, 'Unknown') AS drivers,
	COALESCE(passport, 'Unknown'),
	REGEXP_REPLACE(first, '[0-9]', '', 'g') AS first_name,
	REGEXP_REPLACE(middle, '[0-9]', '', 'g') AS middle_name,
...
SELECT
	id,
	organization,
	REGEXP_REPLACE(name, '[0-9]', '', 'g') AS full_name,
	CASE UPPER(gender)
		WHEN 'M' THEN 'Male'
		WHEN 'F' THEN 'Feale'
		ELSE 'Unknown'
	...
	CAST (encounters AS INT) AS ecounters
FROM raw.providers;
...
CREATE TABLE analytics.fact_claims (
    id TEXT,
    patient_id TEXT,
    provider_id TEXT,
